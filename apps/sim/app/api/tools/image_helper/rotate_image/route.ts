...
const logger = createLogger('RotateImageAPI')

const RotateImageSchema = z.object({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const params = RotateImageSchema.parse(body)

    logger.info(`[${requestId}] Rotating image by ${params.degree} degrees`)

    const { buffer, mimeType } = await loadImageBuffer(params.image)
    const degree = params.degree

    const outBuffer = await sharp(buffer).rotate(degree).toBuffer()
    ...
    logger.info(`[${requestId}] Image rotated successfully to ${meta.width}x${meta.height}`)
    ...
    logger.error(`[${requestId}] Rotate image failed:`, error)
